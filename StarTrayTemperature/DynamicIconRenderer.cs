...
namespace StarTrayTemperature
{
    public static class DynamicIconRenderer
    {
        public static Icon CreateDynamicIcon(Image baseMask, Color iconColorStart, Color iconColorEnd, Color textColor, string text, int width, int height, Font font, int moveX, int moveY)
        {
            ...
            BitmapData gradData = gradientBmp.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
            BitmapData maskData = maskBmp.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
            BitmapData outData = bitmap.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
            ...
            gradientBmp.UnlockBits(gradData);
            maskBmp.UnlockBits(maskData);
            bitmap.UnlockBits(outData);
